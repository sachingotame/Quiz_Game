...
#         if checkValidity(daily_reset_time):
#             Leaderboard.objects.all().update(daily_score=0)

#         if checkValidity(weekly_reset_time):
#             Leaderboard.objects.all().update(weekly_score=0)

#         if checkValidity(monthly_reset_time):
...
class QuizLeaderboardView(APIView):
    def get(self, request):
        ...
        daily_top = Leaderboard.objects.filter(user_id_is_active=True, user_id_is_staff=False,
            user_id__is_superuser=False)\
            .order_by('-daily_score')


        weekly_top = Leaderboard.objects.filter(user_id_is_active=True, user_id_is_staff=False,
            user_id__is_superuser=False)\
            .order_by('-weekly_score')
        monthly_top = Leaderboard.objects.filter(user_id_is_active=True, user_id_is_staff=False,
            user_id__is_superuser=False)\
            .order_by('-monthly_score')
        ...
        data = {
            'daily': [{'username': profile.user_id.username, 'daily_score': profile.daily_points,'total_score': profile.points, 'position': daily_positions[i]}
                    for i, profile in enumerate(daily_top)],
            'weekly': [{'username': profile.user_id.username, 'weekly_score': profile.weekly_points,'total_score': profile.points, 'position':  weekly_positions[i] }
                    for i, profile in enumerate(weekly_top)],
            'monthly': [{'username': profile.user_id.username, 'monthly_score': profile.monthly_points,'total_score': profile.points, 'position':  monthly_positions[i] }
                    for i, profile in enumerate(monthly_top)],
        }
